...
Ứng dụng hỗ trợ sử dụng OpenAI API để trích xuất thông tin chính xác hơn.

### Cách 1: Streamlit Secrets (Khuyến nghị cho deployment)

Thêm API key vào Streamlit Secrets:
- **Local**: Tạo file `.streamlit/secrets.toml` với nội dung:
  ```toml
  OPENAI_API_KEY = "sk-proj-your-api-key-here"
  ```
- **Streamlit Cloud**: Vào Settings → Secrets và thêm secret tương tự

### Cách 2: File config.py (Chỉ cho development local)

Tạo file `config.py` trong thư mục root:
```python
OPENAI_API_KEY = "sk-proj-your-api-key-here"
```

⚠️ **Lưu ý:** File `config.py` đã được thêm vào `.gitignore` và sẽ KHÔNG được commit lên GitHub.
...
- **KHÔNG commit API key** vào GitHub - sử dụng Streamlit Secrets cho deployment
