...
def get_easyocr_reader():
    """Lấy hoặc khởi tạo EasyOCR reader (cache trong session_state)"""
    if 'easyocr_reader' not in st.session_state:
        if EASYOCR_AVAILABLE:
            try:
                st.session_state['easyocr_reader'] = easyocr.Reader(['vi', 'en'], gpu=False)
            except Exception:
                st.session_state['easyocr_reader'] = None
        else:
            st.session_state['easyocr_reader'] = None
    return st.session_state.get('easyocr_reader')

def extract_text_with_ocr(image):
    """Trích xuất text từ ảnh sử dụng OCR (EasyOCR hoặc Tesseract)"""
    # Chuyển đổi PIL Image sang numpy array cho EasyOCR
    image_np = None
    if isinstance(image, Image.Image):
        image_np = np.array(image)
    
    # Thử EasyOCR trước (không cần system dependencies)
    reader = get_easyocr_reader()
    if reader is not None and image_np is not None:
        try:
            ...
            # Fallback to Tesseract nếu EasyOCR lỗi
            pass
    ...
    if TESSERACT_AVAILABLE:
        try:
            ...
            if "tesseract is not installed" in error_msg.lower() or "not in your path" in error_msg.lower():
                # Thử lại EasyOCR nếu Tesseract không có
                if reader is not None and image_np is not None:
                    try:
                        results = reader.readtext(image_np, paragraph=True)
                        text = '\n'.join([result[1] for result in results])
