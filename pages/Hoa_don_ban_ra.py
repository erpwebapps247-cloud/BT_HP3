...
# Khởi tạo EasyOCR reader (nếu có) - chỉ khởi tạo 1 lần
...
# Đọc API key - ưu tiên từ Streamlit Secrets, sau đó từ config
try:
    DEFAULT_API_KEY = st.secrets.get("OPENAI_API_KEY")
except (AttributeError, KeyError, FileNotFoundError):
    # Fallback: đọc từ config (nếu có)
    try:
...
def extract_invoice_info(image):
    """Trích xuất thông tin từ ảnh hóa đơn sử dụng OCR (EasyOCR hoặc Tesseract)"""
    global _easyocr_reader
    
    # Chuyển đổi PIL Image sang numpy array cho EasyOCR
    image_np = None
    if isinstance(image, Image.Image):
        image_np = np.array(image)
    
    # Thử EasyOCR trước (không cần system dependencies, hoạt động tốt trên Streamlit Cloud)
    if EASYOCR_AVAILABLE and _easyocr_reader is not None and image_np is not None:
        try:
            results = _easyocr_reader.readtext(image_np, paragraph=True)
            # Kết hợp tất cả text từ EasyOCR
            text = '\n'.join([result[1] for result in results])
            if text.strip():
                return text
        except Exception as e:
            # Không hiển thị warning, chỉ log để debug
            error_msg = str(e).lower()
            if "invalid input type" not in error_msg:
                # Chỉ hiển thị warning nếu lỗi không phải do input type
                pass
    
    # Fallback: Sử dụng Tesseract nếu có
    if TESSERACT_AVAILABLE:
        try:
            ...
            error_msg = str(e)
            # Không hiển thị error nếu đã thử EasyOCR trước
            if not (EASYOCR_AVAILABLE and _easyocr_reader is not None):
                if "tesseract is not installed" in error_msg.lower() or "not in your path" in error_msg.lower():
                    st.error("""
                    **❌ Lỗi: Tesseract OCR chưa được cài đặt**
                    
                    **Giải pháp:**
                    1. **Khuyến nghị:** Sử dụng OpenAI API để trích xuất thông tin (chính xác hơn, không cần OCR)
                       - Bật "Sử dụng OpenAI API" trong phần cấu hình nâng cao
                       - Nhập API key của bạn
                    
                    2. **Hoặc:** Cài đặt Tesseract OCR trên máy local
                    """)
            return None
    
    # Nếu cả hai đều không khả dụng hoặc lỗi
    return None
...
def load_excel_data():
    """Đọc dữ liệu từ file Excel và format lại file"""
    try:
        ...
        # Format tất cả các hàng dữ liệu
        ...
        column_widths = {'A': 15, 'B': 15, 'C': 60, 'D': 50, 'E': 20}
...
def save_to_excel(new_data):
    ...
    try:
        ...
        # Định dạng lại TẤT CẢ các hàng dữ liệu (từ hàng 2 đến cuối)
        for row_idx in range(2, ws.max_row + 1):
            for col_idx in range(1, len(headers) + 1):
                cell = ws.cell(row_idx, col_idx)
                cell.font = data_font
    ...
    if uploaded_file is not None:
        ...
        with col1:
            ...
            if uploaded_file.type == 'application/pdf':
                ...
                # Thử sử dụng PyMuPDF trước (không cần poppler)
                if PYMUPDF_AVAILABLE:
                    try:
                        pdf_document = fitz.open(stream=pdf_bytes, filetype="pdf")
                        first_page = pdf_document[0]
                        # Tăng chất lượng render: Matrix(3, 3) = DPI 216, Matrix(4, 4) = DPI 288
                        # Sử dụng Matrix(3, 3) để cân bằng chất lượng và tốc độ
                        pix = first_page.get_pixmap(matrix=fitz.Matrix(3, 3), alpha=False)
                        img_data = pix.tobytes("png")
                        image = Image.open(io.BytesIO(img_data))
                        pdf_document.close()
                    except Exception as e:
                        error_msg = f"PyMuPDF error: {str(e)}"
                
                # Fallback: Sử dụng pdf2image nếu PyMuPDF không khả dụng
                if image is None and PDF2IMAGE_AVAILABLE:
                    try:
                        ...
                        error_msg = f"pdf2image error: {str(e)}"
                
                # Xử lý kết quả
                if image:
                    st.image(image, caption="Trang đầu của PDF", use_container_width=True)
                    ...
                    if "poppler" in error_msg.lower() or "Unable to get page count" in error_msg:
                        st.warning("""
                        **⚠️ Lỗi: Poppler chưa được cài đặt**
                        
                        **Giải pháp:**
                        1. Vui lòng chuyển đổi file PDF sang ảnh (PNG/JPG) bằng:
                           - Chụp màn hình PDF
                           - Sử dụng công cụ chuyển đổi online (PDF to Image)
                           - Hoặc xuất PDF sang ảnh từ phần mềm xem PDF
                        2. Sau đó upload file ảnh thay vì file PDF
                        
                        **Lưu ý:** File ảnh (PNG, JPG) vẫn được hỗ trợ đầy đủ cho việc trích xuất thông tin.
                        """)
                    elif error_msg:
